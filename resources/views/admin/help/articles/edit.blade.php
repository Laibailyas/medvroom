<x-admin-layout>
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <form id="article-form" action="{{ route('admin.help.articles.update', $article) }}" method="POST" class="p-8">
            @csrf
            @method('PATCH')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div>
                    ...
                    <x-form.input id="title" name="title" type="text" value="{{ old('title', $article->title) }}" required :error="$errors->has('title')" />
                ...
                <div>
                    ...
                    <x-form.select id="help_category_id" name="help_category_id" :error="$errors->has('help_category_id')">
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('help_category_id', $article->help_category_id) == $category->id ? 'selected' : '' }}>
                ...
                <div class="flex items-center">
                    <label class="flex items-center mt-6">
                        <x-form.checkbox name="is_published" value="1" :checked="old('is_published', $article->is_published)" />
            ...
            <div class="mb-8">
                ...
                <div class="border border-slate-200 rounded-lg overflow-hidden @error('content') border-rose-300 ring-rose-100 @enderror">
                    <div id="editor" class="h-64 border-0">{!! old('content', $article->content) !!}</div>
                </div>
            ...
            <div class="mt-8 pt-6 border-t border-slate-100 flex items-center justify-end space-x-3">
                ...
                <x-button>Update Article</x-button>
            </div>

<x-admin-layout>
    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <form action="{{ route('admin.help.categories.update', $category) }}" method="POST" class="p-8">
            @csrf
            @method('PATCH')
            ...
            <div class="space-y-6 text-sm">
                ...
                <div>
                    ...
                    <x-form.select id="type" name="type" :error="$errors->has('type')">
                        <option value="patient" {{ old('type', $category->type) === 'patient' ? 'selected' : '' }}>Patient</option>
                        <option value="provider" {{ old('type', $category->type) === 'provider' ? 'selected' : '' }}>Provider</option>
                ...
                <div>
                    ...
                    <x-form.textarea id="description" name="description" rows="3" :error="$errors->has('description')">{{ old('description', $category->description) }}</x-form.textarea>
                    <x-form.error :messages="$errors->get('description')" />
                </div>

                <div>
                    <x-form.label for="icon">Icon (Optional)</x-form.label>
                    <x-form.input id="icon" name="icon" type="text" placeholder="e.g. appointment, user, heart" value="{{ old('icon', $category->icon) }}" :error="$errors->has('icon')" />

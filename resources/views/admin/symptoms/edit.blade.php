<x-admin-layout>
    ...
    <div class="max-w-4xl mx-auto pb-12">
        ...
        <form action="{{ route('admin.symptoms.update', $symptom) }}" method="POST">
            @csrf
            @method('PATCH')
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    <div class="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                        ...
                        <a href="{{ route('admin.specialties.index') }}" class="text-[10px] font-black uppercase tracking-widest text-indigo-600 hover:text-indigo-800 transition">Manage Specialties</a>
                    </div>
                    <div class="p-6">
                        <p class="text-xs text-slate-500 mb-6 font-medium leading-relaxed font-medium">Map patient-friendly symptoms to the medical specialties that treat them.</p>
                            ...
                            @foreach($specialties as $specialty)
                                <label class="relative flex items-center p-3 rounded-lg border border-slate-200 cursor-pointer hover:bg-slate-50 transition-colors group">
                                    <input 
                                        ...
                                        {{ in_array($specialty->id, old('specialties', $symptom->specialties->pluck('id')->toArray())) ? 'checked' : '' }}
                                    >
                ...
                <div class="flex items-center justify-end space-x-3">
                    <a href="{{ route('admin.symptoms.index') }}" class="px-6 py-2 text-sm font-bold text-slate-600 hover:text-slate-800 transition">Cancel Changes</a>
                    <x-button>Update Clinical Data</x-button>
                </div>
            </div>
        </form>
    </div>

    ...
    <div class="max-w-3xl mx-auto">
        <div class="mb-6 flex items-center justify-between">
            ...
            <div class="flex items-center space-x-2">
                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold leading-none bg-slate-100 text-slate-500">ID: #{{ $insuranceProvider->id }}</span>
            </div>
        </div>

        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <form action="{{ route('admin.insurance-providers.update', $insuranceProvider) }}" method="POST" enctype="multipart/form-data" class="p-8">
                @csrf
                @method('PATCH')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-6">
                            <div class="shrink-0">
                                <div class="h-24 w-24 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center overflow-hidden p-2 shadow-inner group relative">
                                    @if($insuranceProvider->logo)
                                        <img src="{{ Storage::url($insuranceProvider->logo) }}" alt="{{ $insuranceProvider->name }}" class="max-w-full max-h-full object-contain">
                                    @else
                            ...
                            <label class="block grow">
                                ...
                                <p class="mt-2 text-xs text-slate-400">Leave blank to keep the current logo. PNG, JPG or SVG. Max 2MB.</p>
                            </label>
                        </div>
                        @error('logo')
                            <p class="mt-1 text-xs text-rose-500 font-medium">{{ $message }}</p>
                    ...
                    <div class="flex items-center justify-between p-4 bg-slate-50 rounded-lg border border-slate-100">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            ...
                            <input type="checkbox" name="is_featured" value="1" class="sr-only peer" {{ old('is_featured', $insuranceProvider->is_featured) ? 'checked' : '' }}>

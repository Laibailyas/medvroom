    ...
    <div class="flex h-[calc(100vh-140px)] -m-6 lg:-m-12 overflow-hidden bg-white rounded-[2.5rem] border border-slate-100 shadow-sm">
        ...
        <div class="w-80 md:w-96 flex flex-col border-r border-slate-100 flex-shrink-0 bg-slate-50/30">
            <div class="p-8 border-b border-slate-100">
                <h2 class="text-3xl font-black text-slate-900 tracking-tighter mb-6 uppercase">Patient Chat</h2>
            ...
            <div class="flex-1 overflow-y-auto divide-y divide-slate-50">
                       ...
                       class="flex items-center gap-5 p-6 hover:bg-slate-50 transition-all border-l-4 {{ isset($conversation) && $conversation->id === $conv->id ? 'bg-white border-primary shadow-sm' : 'border-transparent' }}">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex justify-between items-start mb-1">
                                <h4 class="font-black text-slate-900 truncate tracking-tight">{{ $conv->partner?->name ?? 'Unknown' }}</h4>
                                <span class="text-[10px] font-black text-slate-400 whitespace-nowrap uppercase tracking-widest ">{{ $conv->last_message_at ? $conv->last_message_at->shortRelativeDiffForHumans() : '' }}</span>
                            </div>
                            <p class="text-xs {{ isset($conversation) && $conversation->id === $conv->id ? 'text-slate-600' : 'text-slate-400 font-bold' }} truncate ">
                ...
                @empty
                    <div class="p-12 text-center text-slate-300">
                        ...
                        <p class="text-sm font-black uppercase tracking-widest">No Active Chats</p>
                    </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="flex-1 flex flex-col bg-white">
            @if(isset($conversation))
                ...
                <div class="px-10 py-6 border-b border-slate-100 flex items-center justify-between bg-white shadow-sm shrink-0">
                    <div class="flex items-center gap-5">
                        ...
                        <div>
                            <h3 class="font-black text-slate-900 text-lg tracking-tighter">{{ $conversation->partner?->name ?? 'Unknown User' }}</h3>
                            <div class="flex items-center gap-2">
                                <div class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest ">Connection Live</span>
                            </div>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                         <a href="{{ route('doctor.patients.show', $conversation->patient_id) }}" 
                            class="px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-900 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all border border-slate-200 shadow-sm flex items-center gap-2">
            ...
            @else
                ...
                <div class="flex-1 flex flex-col items-center justify-center bg-slate-50/30 p-12 text-center">
                    ...
                    <h2 class="text-4xl font-black text-slate-900 tracking-tighter mb-4">Patient Messaging</h2>
                    <p class="text-slate-400 font-bold max-w-sm ">Communicate securely with your patients. Select a thread on the left to review history or send a new message.</p>
                </div>
            @endif
        </div>
    </div>

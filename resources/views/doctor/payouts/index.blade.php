<x-doctor-layout>
    <div class="space-y-8">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div>
                <h1 class="text-4xl font-black tracking-tighter italic text-slate-900">Earnings & Payouts</h1>
            ...
            @if(!$doctorProfile->payouts_enabled)
                <form action="{{ route('doctor.payouts.connect') }}" method="POST">
                    @csrf
                    <button type="submit" class="px-8 py-4 bg-primary text-slate-900 rounded-[1.5rem] font-black text-sm hover:scale-105 transition-all shadow-xl shadow-primary/20 italic flex items-center gap-2">
            ...
            @else
                <div class="flex items-center gap-4 px-6 py-3 bg-emerald-50 rounded-2xl border border-emerald-100">
                    ...
                    <span class="text-xs font-black text-emerald-600 uppercase tracking-widest italic">Payouts Active</span>
                </div>
            @endif
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl shadow-slate-900/10 relative overflow-hidden group">
                ...
                <div class="relative z-10">
                    <p class="text-[10px] font-black text-slate-500 uppercase tracking-widest italic mb-2">Available for Withdrawal</p>
                    <h2 class="text-5xl font-black italic tracking-tighter text-primary">${{ number_format($availableBalance, 2) }}</h2>
            ...
            <div class="bg-white rounded-[2.5rem] border border-slate-100 p-10 shadow-sm relative overflow-hidden group">
                ...
                <div class="relative z-10">
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest italic mb-2">Pending Clearance</p>
                    <h2 class="text-5xl font-black italic tracking-tighter text-slate-900">${{ number_format($pendingBalance, 2) }}</h2>
        ...
        <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
            <div class="p-8 border-b border-slate-50">
                <h2 class="text-2xl font-black tracking-tighter italic text-slate-900 leading-none">Transaction Record</h2>
                ...
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100">
                            <th class="px-8 py-6 text-[10px] uppercase font-black tracking-widest text-slate-400 italic">Reference ID</th>
                            <th class="px-8 py-6 text-[10px] uppercase font-black tracking-widest text-slate-400 italic">DateTime</th>
                            <th class="px-8 py-6 text-[10px] uppercase font-black tracking-widest text-slate-400 italic">Amount</th>
                            <th class="px-8 py-6 text-[10px] uppercase font-black tracking-widest text-slate-400 italic">Status</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-50">
                        @forelse($payouts as $payout)
                            <tr class="group hover:bg-slate-50/50 transition-colors">
                                <td class="px-8 py-6 font-black text-slate-900 italic tracking-tight">#{{ $payout->id }}</td>
                                <td class="px-8 py-6">
                                    <p class="text-sm font-black italic tracking-tight text-slate-700">{{ $payout->created_at->format('M d, Y') }}</p>
                                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ $payout->created_at->format('g:i A') }}</p>
                                </td>
                                <td class="px-8 py-6">
                                    <span class="text-lg font-black italic tracking-tighter text-slate-900">${{ number_format($payout->amount, 2) }}</span>
                                </td>
                                <td class="px-8 py-6">
                                    @php
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest italic {{ $statusClass }}">
                                        {{ $payout->status }}
                                    </span>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="px-8 py-20 text-center">
                                    ...
                                    <p class="text-slate-400 font-bold italic tracking-tight">No transactional data recorded yet.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            @if($payouts->hasPages())
                <div class="px-8 py-6 bg-slate-50 border-t border-slate-100 italic font-bold">
                    {{ $payouts->links() }}
                </div>
            @endif
        </div>

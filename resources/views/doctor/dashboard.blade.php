<x-doctor-layout>
    <div class="space-y-8">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div>
                <h1 class="text-4xl font-black tracking-tighter ">Workspace Dashboard</h1>
                <p class="text-slate-500 font-bold mt-1">Welcome back, {{ Auth::user()->last_name }}. Here's what's happening today.</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('doctor.schedule.index') }}" class="px-6 py-3 bg-white border border-slate-200 text-slate-900 rounded-2xl font-black text-sm hover:bg-slate-50 transition-all shadow-sm">View Schedule</a>
                <a href="{{ route('doctor.schedule.index') }}" class="px-6 py-3 bg-primary text-slate-900 rounded-2xl font-black text-sm hover:scale-105 transition-all shadow-lg shadow-primary/20">Add Availability</a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-white p-6 md:p-8 rounded-[2rem] border border-slate-100 shadow-sm group hover:ring-2 hover:ring-primary transition-all duration-500">
                ...
                <div class="mt-6">
                    <p class="text-[10px] md:text-xs uppercase font-black tracking-widest text-slate-400 ">Day Forecast</p>
                    <h3 class="text-2xl md:text-3xl font-black tracking-tighter mt-1">$0.00</h3>
            ...
            <div class="bg-white p-6 md:p-8 rounded-[2rem] border border-slate-100 shadow-sm group hover:ring-2 hover:ring-primary transition-all duration-500">
                ...
                <div class="mt-6">
                    <p class="text-[10px] md:text-xs uppercase font-black tracking-widest text-slate-400 ">Today's Visits</p>
                    <h3 class="text-2xl md:text-3xl font-black tracking-tighter mt-1">{{ count($todaysAppointments) }}</h3>
            ...
            <div class="bg-white p-6 md:p-8 rounded-[2rem] border border-slate-100 shadow-sm group hover:ring-2 hover:ring-primary transition-all duration-500">
                ...
                <div class="mt-6">
                    <p class="text-[10px] md:text-xs uppercase font-black tracking-widest text-slate-400 ">Pending Requests</p>
                    <h3 class="text-2xl md:text-3xl font-black tracking-tighter mt-1">{{ $pendingRequests }}</h3>
                    <p class="text-xs font-bold text-slate-400 mt-2">Awaiting your response</p>
                </div>
            </div>
            ...
            <div class="bg-white p-6 md:p-8 rounded-[2rem] border border-slate-100 shadow-sm group hover:ring-2 hover:ring-primary transition-all duration-500">
                ...
                <div class="mt-6">
                    <p class="text-[10px] md:text-xs uppercase font-black tracking-widest text-slate-400 ">Active Patients</p>
                    <h3 class="text-2xl md:text-3xl font-black tracking-tighter mt-1">{{ $totalPatients }}</h3>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col">
                <div class="p-8 border-b border-slate-50 flex items-center justify-between">
                    <div>
                        <h2 class="text-2xl font-black tracking-tighter ">Today's Schedule</h2>
                        <p class="text-xs font-bold text-slate-400 mt-0.5">{{ now()->format('l, F j, Y') }}</p>
                    </div>
                    <a href="#" class="text-xs font-black text-primary uppercase tracking-widest hover:underline ">Full Calendar</a>
                </div>
                    ...
                    @forelse($todaysAppointments as $appointment)
                        <div class="p-8 flex items-center gap-6 group hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0">
                            <div class="text-center w-20 shrink-0">
                                <p class="text-sm font-black tracking-tighter text-slate-400 group-hover:text-primary transition-colors">{{ $appointment->appointment_datetime->format('g:i A') }}</p>
                            </div>
                            ...
                            <div class="shrink-0 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button class="px-4 py-2 bg-slate-900 text-white rounded-xl text-xs font-black transition-all shadow-lg shadow-slate-900/10">View Info</button>
                            </div>
                        </div>
                    @empty
                        <div class="p-20 text-center">
                            ...
                            <p class="text-slate-400 font-bold tracking-tight">No appointments scheduled for today yet.</p>
                        </div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="space-y-8">
                ...
                <div class="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-xl shadow-slate-900/10">
                    <h2 class="text-xl font-black tracking-tighter mb-6">Upcoming Next</h2>
                    ...
                    <div class="space-y-6">
                        @forelse($upcomingAppointments as $appointment)
                            <div class="flex items-center gap-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-black truncate leading-tight">{{ $appointment->patientProfile->user->name }}</p>
                        ...
                        @empty
                            <p class="text-slate-600 text-sm font-bold tracking-tight">No upcoming confirmed visits.</p>
                        @endforelse
                    </div>

                    <a href="#" class="block w-full text-center py-4 bg-white/5 hover:bg-white/10 rounded-2xl mt-8 text-xs font-black uppercase tracking-widest transition-all">Details List</a>
                </div>

                <!-- Messaging Quick Links -->
                <div class="bg-white rounded-[2.5rem] border border-slate-100 p-8 shadow-sm">
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-xl font-black tracking-tighter leading-none">Patient Chat</h2>
                        <div class="w-2 h-2 rounded-full bg-primary animate-pulse"></div>
                    </div>
                    <p class="text-slate-500 text-sm font-bold mb-6">Stay connected with your patients in real-time.</p>
                    <a href="#" class="flex items-center justify-center gap-2 w-full py-4 bg-slate-50 hover:bg-slate-100 rounded-2xl text-xs font-black uppercase tracking-widest transition-all group">

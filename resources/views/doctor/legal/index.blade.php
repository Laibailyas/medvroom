<x-doctor-layout>
    <div class="space-y-8">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div>
                <h1 class="text-4xl font-black tracking-tighter text-slate-900">Legal &amp; Agreements</h1>
                <p class="text-slate-500 font-bold mt-1 uppercase tracking-widest text-[10px]">Documents governing your relationship with MedVroom.</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6">
            @forelse($documents as $document)
                <div class="bg-white rounded-[2.5rem] border border-slate-100 p-8 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div class="flex items-center gap-5">
                        ...
                        <div>
                            <h3 class="text-lg font-black tracking-tighter text-slate-900">{{ $document->title }}</h3>
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">
                                Version {{ $document->version }}
                                @if($document->accepted_at)
                                    &middot; Accepted {{ $document->accepted_at->format('M d, Y g:i A') }}
                                @else
                    ...
                    <div class="flex items-center gap-3 shrink-0">
                        <a href="{{ route('doctor.legal.show', $document->slug) }}"
                        ...
                        <a href="{{ route('doctor.legal.download', $document->slug) }}"
            ...
            @empty
                <div class="p-20 text-center bg-white rounded-[2.5rem] border border-slate-100">
                    <p class="text-slate-400 font-bold tracking-tight">No legal documents available yet.</p>
                </div>
            @endforelse
        </div>
    </div>

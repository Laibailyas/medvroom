...
$classes = ($active ?? false)
            ? 'flex items-center gap-3 px-4 py-3 bg-primary text-slate-900 rounded-2xl font-black text-sm shadow-lg shadow-primary/20 transition-all duration-300'
            : 'flex items-center gap-3 px-4 py-3 text-slate-400 hover:bg-white/5 hover:text-white rounded-2xl font-bold text-sm transition-all duration-300 group';
...
$iconSvg = match($icon) {
    ...
    default => '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>',
};
@endphp

<a {{ $attributes->merge(['class' => $classes]) }}>
    <div class="{{ ($active ?? false) ? 'text-slate-900' : 'text-slate-500 group-hover:text-primary transition-colors' }}">
        {!! $iconSvg !!}
    </div>
    <span>{{ $slot }}</span>
</a>
